...
plist=['Halibut Bank',]
varNameDict={'Halibut Bank':'HalibutBank','Sentry Shoal':'SentryShoal', 'S3':'S3', 'Central node':'CentralNode', 'Central SJDF':'CentralSJDF','all':'All'}
#plist=['Sentry Shoal','S3','Central node','Central SJDF']
t0=dt.datetime(2015,1,1)
fdur=1 # length of each results file in days
dirname='hindcast2015P'
te=dt.datetime(2016,1,1)

def setup():
    spath='/results2/SalishSea/hindcast/'
    ffmt='%Y%m%d'
    dfmt='%d%b%y'
    ...
    while iits<=te:
        ...
        try:
            ...
            raise
        fnames['tempBase'][ind]=saveloc+'temp2/ptrc'+iits.strftime(ffmt)+'-'+iite.strftime(ffmt)
        iits=iitn
        ind=ind+1
...
def runExtractPtrc():
    ...
    for ii in range(0,fnum):
        ...
        pids[ii]=subprocess.Popen('ncks -v vosaline,votemper '+fnames['ptrc_T'][ii]+' '+f0, shell=True,
...
def runExtractLocs():
    ...
    for pl in plist:
        ...
        for ii in range(0,fnum):
            ...
            pids[ii]=subprocess.Popen('ncks -v vosaline,votemper,e3t -d x,'+str(i)+' -d y,'+str(j)+' '+f0+' '+fpl, shell=True,
                                           stdout=subprocess.PIPE,  stderr=subprocess.PIPE)
            if ii%maxproc==(maxproc-1):
                pids[ii].wait() # wait for the last one in set
        pids[ii].wait() # wait for the last one

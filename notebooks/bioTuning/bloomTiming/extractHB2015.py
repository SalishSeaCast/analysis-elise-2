...
fields=['diatoms','flagellates','ciliates','nitrate','Fraser_tracer']
t0=dt.datetime(2015,1,1)
fdur=1 # length of each results file in days
dirname='hindcast2015'
...
def runExtractPtrc():
    ...
    for ii in range(0,fnum):
        ...
        pids[ii]=subprocess.Popen('ncks -v '+','.join(fields)+' '+fnames['ptrc_T'][ii]+' '+f0, shell=True,
...
def runExtractLocs():
    ...
    for pl in plist:
        ...
        for ii in range(0,fnum):
            ...
            pids[ii]=subprocess.Popen('ncks -v '+','.join(fields)+',e3t -d x,'+str(i)+' -d y,'+str(j)+' '+f0+' '+fpl, shell=True,
                                           stdout=subprocess.PIPE,  stderr=subprocess.PIPE)
            if ii%maxproc==(maxproc-1):
                pids[ii].wait() # wait for the last one in set
        pids[ii].wait() # wait for the last one

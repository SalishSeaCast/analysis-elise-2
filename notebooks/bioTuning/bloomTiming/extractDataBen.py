...
def runExtractPtrc(ilist,fnames):
    ...
    for ii in ilist:
        ...
        pids[ii]=subprocess.Popen('ncks -d x,'+str(i)+' -d y,'+str(j)+' -d deptht,0 -v '+','.join(fields)+' '+fnames['ptrc_T'][ii]+' '+f0, shell=True,
...
def runExtractLocs(ilist,fnames):
    ...
    for pl in plist:
        ...
        for ii in ilist:
            ...
            pids[ii]=subprocess.Popen('ncks -d x,'+str(i)+' -d y,'+str(j)+' -d deptht,0 -v '+','.join(fields)+' '+fnames['ptrc_T'][ii]+' '+fpl, shell=True,
                                           stdout=subprocess.PIPE,  stderr=subprocess.PIPE)

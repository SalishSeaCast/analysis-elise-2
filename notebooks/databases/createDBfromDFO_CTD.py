...
def main():
    ...
    class AncillaryTBL(Base):
        __table__=Table('AncillaryTBL', Base.metadata,
                    Column('StationTBLID', Integer, primary_key=True),
                    ...
                    Column('xmiss_Serial', String),
                    Column('xmiss_CalDate', String),
                    Column('xmiss_m', forceNumeric),
                    Column('xmiss_b', forceNumeric),
                    Column('xmiss_PathLen', forceNumeric),
                    Column('PAR_Serial', String),
                    Column('PAR_CalDate', String),
                    Column('PAR_m', forceNumeric),
                    Column('PAR_b', forceNumeric),
                    Column('PAR_CalConst', forceNumeric),
                    Column('PAR_Multiplier', forceNumeric),
                    Column('PAR_Offset', forceNumeric),
                    Column('RefPAR_Serial', String),
                    Column('RefPAR_CalDate', String),
                    Column('RefPAR_ConvFact', forceNumeric),
                    Column('RefPAR_Multiplier', forceNumeric))
    ...
    # compile some regular expressions for later use
    ...
    PARPatterns={'PAR_Serial':re.compile('(?<=SerialNumber>).*(?=</SerialNumber)'),
                   'PAR_CalDate':re.compile('(?<=CalibrationDate>).*(?=</CalibrationDate)'),
                   'PAR_m':re.compile('(?<=M>).*(?=</M)'),
                   'PAR_b':re.compile('(?<=B>).*(?=</B)'),
                   'PAR_CalConst':re.compile('(?<=CalibrationConstant>).*(?=</CalibrationConstant)'),
                   'PAR_Multiplier':re.compile('(?<=Multiplier>).*(?=</Multiplier)'),
                   'PAR_Offset':re.compile('(?<=Offset>).*(?=</Offset)')}
    
    SPARPatterns={'RefPAR_Serial':re.compile('(?<=SerialNumber>).*(?=</SerialNumber)'),
                   'RefPAR_CalDate':re.compile('(?<=CalibrationDate>).*(?=</CalibrationDate)'),
                   'RefPAR_ConvFact':re.compile('(?<=ConversionFactor>).*(?=</ConversionFactor)'),
                   'RefPAR_Multiplier':re.compile('(?<=RatioMultiplier>).*(?=</RatioMultiplier)')}
    ...
    for ifile in filenames:
        ...
        ancData={'StationTBLID':stationNo,}
        with open(ifile, 'rt', encoding = "ISO-8859-1") as f:
                ...
                if infile:
                    if re.match('\s*\$', line) or len(line)==0:
                        ...
                        if re.match('START TIME',splitline[0]):
                            ...
                            ancData['DATA_DESCRIPTION']=splitline[1]
                if invars:
                    ...
                    if len(line)==0:
                        ...
                        if re.match('MISSION',splitline[0]):
                            ancData['MISSION']=splitline[1]
                        elif re.match('AGENCY',splitline[0]):
                            ancData['AGENCY']=splitline[1]
                        elif re.match('COUNTRY',splitline[0]):
                            ancData['COUNTRY']=splitline[1]
                        elif re.match('PROJECT',splitline[0]):
                            ancData['PROJECT']=splitline[1]
                        elif re.match('SCIENTIST',splitline[0]):
                            ancData['SCIENTIST']=splitline[1]
                    ...
                    if len(line)==0:
                        ...
                        if re.match('TYPE',splitline[0]):
                            ancData['TYPE']=splitline[1]
                        elif re.match('MODEL',splitline[0]):
                            ancData['MODEL']=splitline[1]
                        elif re.match('SERIAL',splitline[0]):
                            ancData['SERIAL']=splitline[1]
                elif incom:
                    if incomX:
                        for key, pattern in XmissPatterns.items():
                            ...
                            if match is not None:
                                ancData[key]=match[0]
                                break # exit for loop; don't try any more patterns on this line
                    if incomPAR:
                       for key, pattern in PARPatterns.items():
                            match=pattern.search(line)
                            if match is not None:
                                ancData[key]=match[0]
                                break # exit for loop; don't try any more patterns on this line
                    if incomSPAR:                        
                        for key, pattern in SPARPatterns.items():
                            match=pattern.search(line)
                            if match is not None:
                                ancData[key]=match[0]
                                break # exit for loop; don't try any more patterns on this line
            ...
            session.execute(AncillaryTBL.__table__.insert().values(**ancData))
    session.commit()
    engine.dispose()
    fout.close()
    ferr.close()

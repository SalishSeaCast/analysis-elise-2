...
dirname='HC201905spinup2015_1'
te=dt.datetime(2015,3,30)
...
def setup():
    spath='/results/SalishSea/spinup.201905.firsttime/'
    ffmt='%Y%m%d'
    dfmt='%d%b%y'
...
def runExtractLocs():
    ...
    for pl in plist:
        ...
        for ii in range(0,fnum):
            ...
            pids[ii]=subprocess.Popen('ncks -v '+','.join(evars)+',e3t -d x,'+str(i)+' -d y,'+str(j)+' '+f0+' '+fpl, shell=True,
                                           stdout=subprocess.PIPE,  stderr=subprocess.PIPE)
            if ii%maxproc==(maxproc-1):
                pids[ii].wait() # wait for the last one in set
        pids[ii].wait() # wait for the last one

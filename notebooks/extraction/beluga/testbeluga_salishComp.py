...
#################
maxproc=5
saveloc='/data/eolson/results/MEOPAR/SS36runs/calcFiles/comparePhytoN/'
#Aloc='/scratch/eolson/results/calcs/Area_240.nc'
######################

def setup():
    print(dirname)
    spath='/data/eolson/results/MEOPAR/SS36runs/BelugaRuns/'+dirname+'/'
    print(spath)
...
def runExtractLocsFromPtrcAllVar():
    ...
    for pl in plist:
        ...
        for ii in range(0,fnum):
            ...
            pids[ii]=subprocess.Popen(shlex.split('ncks -d x,'+str(i)+' -d y,'+str(j)+' '+f0+' '+fpl), 
...
def runExtractLocsFromPtrc_evars():
    ...
    for pl in plist:
        ...
        for ii in range(0,fnum):
            ...
            fpl=fnames['tempBase'][ii]+varNameDict[pl]+'.nc'
            j,i=places.PLACES[pl]['NEMO grid ji']
            pids[ii]=subprocess.Popen(shlex.split('ncks -v '+','.join(evars)+' -d x,'+str(i)+' -d y,'+str(j)+' '+f0+' '+fpl),
                                           stdout=subprocess.PIPE,  stderr=subprocess.PIPE)
            if ii%maxproc==(maxproc-1):
                pids[ii].wait() # wait for the last one in set
        pids[ii].wait() # wait for the last one
...
def runJoinLocs():
    ...
    for pl in plist:
        ...
        pids[jj]=subprocess.Popen(shlex.split('ncrcat '+' '.join(fpllist)+' '+f1), 
                                           stdout=subprocess.PIPE,  stderr=subprocess.PIPE)

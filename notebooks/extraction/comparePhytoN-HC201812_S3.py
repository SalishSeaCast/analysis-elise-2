...
ptrcexpath='/results/SalishSea/nowcast-green.201812/05jul15/SalishSea_1h_20150705_20150705_ptrc_T.nc'
plist=['S3',]
...
def setup():
    spath='/results/SalishSea/nowcast-green.201812/'
    ffmt='%Y%m%d'
    dfmt='%d%b%y'
    stencilp='{0}/SalishSea_1h_{1}_{1}_ptrc_T.nc'
    stencile='{0}/SalishSea_1h_{1}_{1}_aux_T_.nc'
    ...
    while iits<=te:
        ...
        try:
            ...
            print('file does not exist:  '+spath+stencil.format(iits.strftime(dfmt).lower(),iits.strftime(ffmt),iite.strftime(ffmt)))
            raise
...
def runExtractPtrc():
    ...
    for ii in range(0,fnum):
        ...
        pids[ii]=subprocess.Popen('ncks -v diatoms,flagellates,ciliates,nitrate '+fnames['ptrc_T'][ii]+' '+f0, shell=True,
...
def runExtractLocs():
    ...
    for pl in plist:
        ...
        for ii in range(0,fnum):
            ...
            print('ncks -v diatoms,flagellates,ciliates,e3t -d x,'+str(i)+' -d y,'+str(j)+' '+f0+' '+fpl)
            pids[ii]=subprocess.Popen('ncks -v diatoms,flagellates,ciliates,nitrate -d x,'+str(i)+' -d y,'+str(j)+' '+f0+' '+fpl, shell=True,
                                           stdout=subprocess.PIPE,  stderr=subprocess.PIPE)
            if ii%maxproc==(maxproc-1):
                pids[ii].wait() # wait for the last one in set
        pids[ii].wait() # wait for the last one

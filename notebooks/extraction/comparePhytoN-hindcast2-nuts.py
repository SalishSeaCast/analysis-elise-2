...
def runExtractPtrc():
    ...
    for ii in range(0,fnum):
        ...
        pids[ii]=subprocess.Popen('ncks -v nitrate,silicon,ammonium '+fnames['ptrc_T'][ii]+' '+f0, shell=True,
...
def runExtractLocs():
    ...
    for pl in plist:
        ...
        for ii in range(0,fnum):
            ...
            pids[ii]=subprocess.Popen('ncks -v nitrate,silicon,ammonium,e3t -d x,'+str(i)+' -d y,'+str(j)+' '+f0+' '+fpl, shell=True,
                                           stdout=subprocess.PIPE,  stderr=subprocess.PIPE)
            if ii%maxproc==(maxproc-1):
                pids[ii].wait() # wait for the last one in set
        pids[ii].wait() # wait for the last one
